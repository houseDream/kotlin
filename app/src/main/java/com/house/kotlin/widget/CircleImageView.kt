package com.house.kotlin.widget
...
/**
 * +----------------------------------------------------------------------
 * | @time 2018/4/26:下午7:48
 * | @author house
 * | @class_describe 圆角ImageView
 * +----------------------------------------------------------------------
 **/
class CircleImageView(context: Context, attrs: AttributeSet) : android.support.v7.widget.AppCompatImageView(context, attrs) {
    ...
    init {
        ...
        mType = ta.getInt(R.styleable.CircleImageView_imageType, DEFAULT_TYPE)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (rawBitmap != null && mType != TYPE_NONE) {
            ...
            if (mType == TYPE_CIRCLE) {
                ...
                val borderRadius = if (mRectRoundRadius - halfBorderWidth > 0.0f) (mRectRoundRadius - halfBorderWidth).toFloat() else 0.0f
